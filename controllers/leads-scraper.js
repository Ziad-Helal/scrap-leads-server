...
async function sendMultiRequests(endPoint, response, params) {
  ...
  let { meta, data } = await sendScrappingRequest(
    ...
    response,
    {
      ...params,
      cursor:
        "eyJnbWFwX3BsYWNlX2lkIjo3MDI5MzE0NywiX3BvaW50c1RvTmV4dEl0ZW1zIjp0cnVlfQ",
    },
    c
  );
...
async function sendScrappingRequest(url, response, params, counter) {
  ...
  console.log(
    `${counter * result.meta.per_page + 22500} / ${result.meta.count}\n${
      result.meta.count - counter * result.meta.per_page - 22500
    } remaining...`
  );
